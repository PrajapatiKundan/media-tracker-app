...
export class MockXHRBackend implements HttpBackend {
    ...
    return new Observable((responseObserver: Observer<HttpResponse<any>>) => {
      ...
      switch (request.method) {
        case 'GET':
          ...
          break;
        case 'POST':
          const newMediaItem = request.body;
          newMediaItem.id = this._getNewId();
          this.mediaItems.push(newMediaItem);
          responseOptions = { status: 201 };
          break;
        case 'DELETE':
          const itemId = parseInt(request.url.split('/')[1], 10);
          this._deleteMediaItem(itemId);
  ...
  _deleteMediaItem(id: any) {
    const mediaItem: MediaItem | undefined = this.mediaItems.find(
      (i) => i.id === id
    );
